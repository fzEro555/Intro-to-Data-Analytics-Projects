...
def retrieve_articles():
    ...
    all_articles.extend(retrieve(base_url, api_key, queries[0], "2017", month_ranges))
    # search for hurricanes harvey
    all_articles.extend(retrieve(base_url, api_key, queries[1], "2017", month_ranges))
    # search for hurricanes maria
    all_articles.extend(retrieve(base_url, api_key, queries[2], "2017", month_ranges))
    # search for hurricanes irene
    all_articles.extend(retrieve(base_url, api_key, queries[3], "2011", month_ranges))
    # save to csv
    with open("../basic_analysis/guardian_data.csv", 'w') as file_save:
